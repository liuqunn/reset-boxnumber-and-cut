    ...
    for i in range(len(lists)):
        ...
        if p1 > p2:
            ...
            if (point1[1]-point2[1])/(point1[0]-point2[0])>0:
                ...
                #print('44444444')
                if angle < 82 and angle > 62:  #62  82 根据异常样本按需调整
                    length = int(p2)
                    width = int(p1)
                    angle = angle-90
        ...
        #判断旋转后长和宽 
        if masked.shape[:2][0]> masked.shape[:2][1]:
            width1 = masked.shape[:2][1]
            length1 = masked.shape[:2][0]
        else:
            length1 = masked.shape[:2][1]
            width1 = masked.shape[:2][0]
        imgRotation = cv2.warpAffine(masked, rotateMat, (length1,width1))
        #裁剪图片
        left_x = int(center_x- length/2)
        right_x = int(center_x + length/2)
        bottom_y = int(center_y - width/2)
        top_y = int(center_y + width/2)
        last_img = imgRotation[bottom_y:top_y,left_x:right_x]
        #保存路径，C:/Users/zh/Desktop/rty/
        cv2.imwrite('C:/Users/zh/Desktop/rty/'+label+'_'+picname,last_img)
...
if __name__=="__main__":
    #要读取的文件夹路径
    dir = "C:/Users/zh/Desktop/humen.door/"
